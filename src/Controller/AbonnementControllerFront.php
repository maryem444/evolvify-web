...
final class AbonnementControllerFront extends AbstractController
{
    #[Route('/abonnement_front', name: 'abonnement_list_front')]
public function listAbonnements(AbonnementRepository $abonnementRepository): Response
{
    ...
    foreach ($abonnements as $abonnement) {
        $type = ucfirst(strtolower($abonnement->getType_Ab())); // pour normaliser
        if (isset($typesStats[$type])) {
            $typesStats[$type]++;
        }

        if ($abonnement->getStatus() && $abonnement->getStatus()->value === 'ACTIF') {
            $actifs++;
        }
    }
    ...
    foreach ($typesStats as $type => $count) {
        $pourcentagesTypes[$type] = $total > 0 ? round(($count / $total) * 100) : 0;
    }

    return $this->render('abonnement/list_employe.html.twig', [
        'abonnements' => $abonnements,
        'tauxActivite' => $tauxActivite,
        'typesStats' => $typesStats,
        'pourcentagesTypes' => $pourcentagesTypes,
